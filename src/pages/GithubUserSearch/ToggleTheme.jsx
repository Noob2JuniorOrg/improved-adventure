...
function ToggleTheme() {
  ...
  const themeClasses = ctx.lightTheme
    ? 'fill-github-lightmain text-github-lightmain hover:text-black hover:fill-black'
    : 'fill-white text-white hover:fill-github-lightmain hover:text-github-lightmain';
  const word = ctx.lightTheme ? 'dark' : 'light';
  const icon = ctx.lightTheme ? (
    <svg
  ...
  return (
    <div
      ...
      className={`flex justify-between items-center w-[78px] ${themeClasses}`}
    >
      <p className="uppercase font-bold tracking-[2.5px] text-[13px]">{word}</p>
      {icon}
    </div>
  );
}
